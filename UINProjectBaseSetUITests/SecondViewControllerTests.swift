...
class SecondViewControllerTests: XCTestCase {
    ...
    func testExample() {
        ...
        image.swipeUp()
        image.swipeDown()
        app.otherElements.containingType(.NavigationBar, identifier:"UINProjectBaseSet.SecondView").childrenMatchingType(.Other).element.childrenMatchingType(.Other).element.childrenMatchingType(.Other).element.childrenMatchingType(.Other).elementBoundByIndex(1).childrenMatchingType(.Button).matchingIdentifier("Button").elementBoundByIndex(0).tap()
        image.swipeDown()
    }
